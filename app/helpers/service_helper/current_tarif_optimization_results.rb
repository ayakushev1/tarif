class ServiceHelper::CurrentTarifOptimizationResults
  ...
  attr_reader :service_ids_to_calculate, :tarif_results, :tarif_results_ord, :prev_service_call_ids, :prev_service_group_call_ids, :prev_service_call_ids_by_parts#, :calls_count_by_parts
...
#    @operator_id = operator#tarif_list_generator.operators[operator_index]
  ...
  def process_tarif_results_batch_tarif_resuts(tarif_set_id, tarif_class_id, part, stat, price_formula_order)
    ...
    if price_formula_order == 0          
      ...
      #raise(StandardError, current_tarif_results_ord.attributes.keys) #if tarif_results[tarif_set_id][part][key]
      tarif_results[tarif_set_id][part][tarif_class_id]['call_id_count'] += current_tarif_results_ord['call_id_count'] #if tarif_optimizator.output_call_count_to_tarif_results
      tarif_results[tarif_set_id][part][tarif_class_id]['price_value'] += (current_tarif_results_ord['price_value'] || 0)
      tarif_results[tarif_set_id][part][tarif_class_id]['price_values'] << (current_tarif_results_ord['price_values'] || 0)
    end
  end
  ...
  def process_tarif_results_batch_prev_calls(tarif_set_id, tarif_class_id, part, stat, price_formula_order)
    ...
    if price_formula_order == 0              
      ...
      prev_service_call_ids[tarif_set_id][part][tarif_class_id] += current_tarif_results_ord['call_ids'] #if tarif_optimizator.output_call_ids_to_tarif_results
    end
...
#TODO разобраться с price_formula
    ...
    end unless current_tarif_results_ord['price_values'].blank? 
  end
...
#    raise(StandardError, tarif_results[tarif_set_id][part]) if part == "own-country-rouming/calls" and tarif_set_id == "276_200"
          ...
          if tarif_results[prev_tarif_set_id] and tarif_results[prev_tarif_set_id][part]
            set_current_tarif_results_ord(tarif_set_id, prev_tarif_set_id, service_id)
      ...
      tarif_results.keys.sort{|t| t.split('_').size}.each do |tarif_set_id|
        ...
        while current_tarif_set_size > 0 and !missed_parts.blank?
          ...
          tarif_results[current_tarif_set].each do |part, prev_tarif_result_by_part|
            ...
            prev_tarif_result_by_part.each do |key, prev_tarif_result|
              tarif_results[tarif_set_id][part] ||= {} 
              tarif_results[tarif_set_id][part][key] = prev_tarif_result if true #tarif_results[tarif_set_id][part][key].blank?
            end
          ...
          tarif_results_ord[current_tarif_set].each do |part, prev_tarif_results_by_part|
            ...
            prev_tarif_results_by_part.each do |key_ord, prev_tarif_results_ord_val|
              tarif_results_ord[tarif_set_id][part][key_ord] ||= {}
              prev_tarif_results_ord_val.each do |key, prev_tarif_result_ord| 
                tarif_results_ord[tarif_set_id][part][key_ord][key] = prev_tarif_result_ord if false#tarif_results_ord[tarif_set_id][part][key_ord][key].blank? 
              end
            end
          end if tarif_results_ord[current_tarif_set]
