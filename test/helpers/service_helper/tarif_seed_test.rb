...
describe ServiceHelper::TarifSeedTester do
  ...
  before do
    unless Foo.class_variable_get(:@@init_tarif_optimizator_for_tarif_seed_test)
      options = {:operators => [1030], :tarifs => [[203]], :tarif_sets => [[[203]]], :common_services => [[[]]], :tarif_options => [[[[296]]]]}
#      options = {:operators => [1030], :tarifs => [[203]], :tarif_sets => [[[203, 276, 277]]], :common_services => [[[276, 277]]], :tarif_options => [[[[296]]]]}
    ...
    @tarif_set_id = '203_296'
#    @tarif_set_id = '203_276_277_296'
    @tarif_id = 203
  end
...
#          tarif_classes_category_where_hash_sql.must_be :==, true if tarif_result_detail['service_category_name'] == '_sctcg_mts_sic_1_calls_to_russia'
          ...
          if tarif_result_detail['call_id_count'] == 0 
            ...
            when 'one_time_tarif_switch_on'
              tarif_result_detail['price_value'].must_be :==, 30
            when 'periodic_monthly_fee'
              tarif_result_detail['price_value'].must_be :==, 400
            when 'periodic_day_fee'
              tarif_result_detail['price_value'].must_be :==, 150
            else
              tarif_classes_category_where_hash_sql.must_be :==, true, [tarif_result_detail]
            end
          end
        end  
      end
      ...
      @tarif_set_id.split('_').each do |tarif_id|
        @tarif_optimizator.tarif_results_ord[@tarif_set_id][tarif_id.to_i].each do |key, details_by_order|
          details_by_order['price_values'].each do |tarif_result_detail|
            ...
            sum += tarif_result_detail['call_id_count']
            ids_to_add = (tarif_result_detail['call_ids'] || []).flatten.compact
            ids_and_names << [ids_to_add, tarif_result_detail['service_category_name']]
#            raise(StandardError, [tarif_result_detail, tarif_classes_category_where_hash_sql]) if tarif_result_detail['service_category_name'] == '_sctcg_mts_europe_sms_outcoming'
            (ids_to_add & call_ids).count.must_be :==, 0, [ids_to_add, ids_and_names, tarif_classes_category_where_hash_sql, tarif_result_detail['service_category_name']]
            (ids_to_add & call_ids).count.must_be :==, 0, [ids_to_add, tarif_classes_category_where_hash_sql, tarif_result_detail['service_category_name']]
            call_ids = (call_ids + ids_to_add).flatten.compact
          end  
        end
      end
      ids_not_covered = Customer::Call.pluck(:id) - call_ids
      Customer::Call.count.must_be :==, sum, ids_not_covered
    end
    ...
    it 'should not be service_id in tarif_set results that are not in tarif_set_ids' do
      @tarif_optimizator.tarif_results.each do |key, tarif_set|
        tarif_set.keys.count.must_be :==, key.split('_').count, [@tarif_optimizator.tarif_results.keys, key, tarif_set.keys]
      end      
