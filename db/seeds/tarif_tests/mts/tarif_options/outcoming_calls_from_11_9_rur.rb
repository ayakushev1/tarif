...
other_country_id_1 = _egypt; other_country_operator_id_1 = Relation.country_operator(other_country_id_1); raise(StandardError) unless other_country_operator_id_1 
other_country_id_2 = _usa; other_country_operator_id_2 = Relation.country_operator(other_country_id_2); raise(StandardError) unless other_country_operator_id_2 
...
call_duration = 60 * 10; sms_count = 1; internet_duration = 1;
...
c[:c_other_country_1] = {:operator_id => other_country_operator_id_1, :region_id => nil, :country_id =>  other_country_id_1}
c[:c_other_country_2] = {:operator_id => other_country_operator_id_2, :region_id => nil, :country_id =>  other_country_id_2}
...
calls = []; i = 0
total_steps =  (1 * 1 * 1 * p.size * c.size) + (1 * 1 * 1 * p.size * c.size) + 1
...
1.times do |iii|
  [_calls].each do |service_id|
    [_outbound].each do |direction_id|
      p.each do |partner_key, partner|
        c.each do |connect_key, connect|
          calls << {:base_service_id => service_id, :base_subservice_id => direction_id, :partner_phone => connect.merge({:operator_type_id => _mobile, :number => p_phone}), 
            :connect => connect, :description => {:time => set_date_time(start_date, i, total_steps), :duration => call_duration, :volume => sms_count}, :user_id => user, :own_phone => own_phone}
          i += 1; 
        end
      end
    end
