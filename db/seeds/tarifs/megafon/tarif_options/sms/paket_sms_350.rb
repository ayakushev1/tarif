...
#mms included in tarif
  scg_mgf_paket_sms_350 = @tc.add_service_category_group(
    ...
    {:calculation_order => 0, :price => 400.0, :price_unit_id => _rur, :volume_id => _call_description_volume, :volume_unit_id => _item, :description => '', 
     :formula => {
       :window_condition => "(350 >= count_volume)", :window_over => 'month',
       ...
       :multiple_use_of_tarif_option => {
         :group_by => 'month',
         :stat_params => {:tarif_option_count_of_usage => "ceil(count(description->>'volume') / 350.0)",
