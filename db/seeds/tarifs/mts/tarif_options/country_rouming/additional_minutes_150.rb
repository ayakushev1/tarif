...
@tc.create_tarif_class({
  ...
  :dependency => {
    ...
    :prerequisites => [_mts_smart, _mts_smart_plus],
...
#Добавление новых service_category_group for smart+
  ...
  scg_mts_additional_minutes_150_included_in_tarif_calls = @tc.add_service_category_group(
    ...
    {:calculation_order => 0, :standard_formula_id => _stf_price_by_sum_duration_minute, 
      :formula => {:window_condition => "(150.0 >= sum_duration_minute)", :window_over => 'month'}, :price => 0.0, :description => '' }
    )
...
#All own country regions, Calls, Outcoming, to_own_country, to_own_operator
  @tc.add_grouped_service_category_tarif_class(_sctcg_all_own_country_regions_calls_to_own_country_own_operator, scg_mts_additional_minutes_150_included_in_tarif_calls[:id]) 
