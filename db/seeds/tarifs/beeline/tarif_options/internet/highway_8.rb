...
@tc.create_tarif_class({
  ...
  :features => {:http => 'http://moskva.beeline.ru/customers/products/mobile/services/details/highway-8-gb-tv/'},
  :dependency => {
     ...
     :formula => {
       ...
       :auto_turbo_buttons  => {
         ...
         :stat_params => {
           ...
           :count_of_usage_of_1000 => "GREATEST(ceil((sum((description->>'volume')::float) - 8000.0) / 1000.0), 0.0)",
           :count_of_usage_of_3000 => "GREATEST(ceil((sum((description->>'volume')::float) - 8000.0) / 3000.0), 0.0)"},
