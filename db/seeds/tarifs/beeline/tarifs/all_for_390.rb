...
@tc.create_tarif_class({
  :id => _bln_all_for_390, :name => 'Всё за 390', :operator_id => _beeline, :privacy_id => _person, :standard_service_id => _tarif,
  :features => {:http => 'http://moskva.beeline.ru/customers/products/mobile/tariffs/details/vse-za-390/'},
  :dependency => {
    :incompatibility => {},
...
#Добавление новых service_category_group
  ...
  scg_bln_all_for_390_calls = @tc.add_service_category_group(
    {:name => 'scg_bln_all_for_390_calls' }, 
    {:name => "price for scg_bln_all_for_390_calls"}, 
    {:calculation_order => 0, :standard_formula_id => _stf_price_by_sum_duration_minute, 
      :formula => {:window_condition => "(60.0 >= sum_duration_minute)", :window_over => 'month'}, :price => 0.0, :description => '' }
    )
  #sms included in tarif
  scg_bln_all_for_390_sms = @tc.add_service_category_group(
    {:name => 'scg_bln_all_for_390_sms' }, 
    {:name => "price for scg_bln_all_for_390_sms"}, 
    {:calculation_order => 0, :standard_formula_id => _stf_zero_count_volume_item, 
      :formula => {:window_condition => "(300 >= count_volume)", :window_over => 'month'}, :price => 0.0, :description => '' }
    )

  #internet included in tarif
  scg_bln_all_for_390_internet = @tc.add_service_category_group(
    {:name => 'scg_bln_all_for_390_internet' }, 
    {:name => "price for scg_bln_all_for_390_internet"}, 
...
category = {:name => '_sctcg_own_home_regions_calls_to_own_home_regions_not_own_operator', :service_category_rouming_id => _own_and_home_regions_rouming, :service_category_calls_id => _calls_out, :service_category_geo_id => _service_to_own_and_home_regions, :service_category_partner_type_id => _service_to_not_own_operator}
  @tc.add_grouped_service_category_tarif_class(category, scg_bln_all_for_390_calls[:id])
...
#Own and home regions, Calls, Outcoming, to_bln_international_7 (на другие телефоны стран СНГ (кроме Азербайджана, Беларуси и Молдовы), Грузии, Абхазии, Южной Осетии)
category = {:name => '_sctcg_own_home_regions_calls_to_bln_international_7', :service_category_rouming_id => _own_and_home_regions_rouming, :service_category_calls_id => _calls_out, :service_category_geo_id => _sc_service_to_bln_international_7}
  @tc.add_one_service_category_tarif_class(category, {}, 
...
category = {:name => '_sctcg_own_home_regions_sms_to_own_home_regions', :service_category_rouming_id => _own_and_home_regions_rouming, :service_category_calls_id => _sms_out, :service_category_geo_id => _service_to_own_and_home_regions}
  @tc.add_grouped_service_category_tarif_class(category, scg_bln_all_for_390_sms[:id])
...
#Own and home regions, Internet
  ...
  @tc.add_grouped_service_category_tarif_class(category, scg_bln_all_for_390_internet[:id])
  @tc.add_one_service_category_tarif_class(category, {}, 
