...
scg_tele_add_speed_3gb = @tc.add_service_category_group(
    {:name => 'scg_tele_add_speed_3gb' }, 
    {:name => "price for scg_tele_add_speed_3gb"}, 
    {:calculation_order => 0, :price => 150.0, :price_unit_id => _rur, :volume_id => _call_description_volume, :volume_unit_id => _m_byte, :name => 'stf_tele_add_speed_3gb', :description => '', 
     :formula => {
       ...
       :multiple_use_of_tarif_option => {
         :group_by => 'month',
         :stat_params => {:tarif_option_count_of_usage => "ceil(sum((description->>'volume')::float) / 3000.0)", :sum_volume => "sum((description->>'volume')::float)"},
...
#Own and home regions, Internet
  ...
  @tc.add_grouped_service_category_tarif_class(category, scg_tele_add_speed_3gb[:id])
